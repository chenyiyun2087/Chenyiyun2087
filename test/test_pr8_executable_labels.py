...
class TestExecutableLabels:
    def test_computes_all_horizons(self, prices):
        result = compute_executable_forward_returns(prices, hold_days=10)
    ...
    def test_returns_have_cost_subtracted(self, prices):
        result = compute_executable_forward_returns(prices, hold_days=10, cost_rate=0.002)
        # Spot check: mean returns should be slightly lower with higher costs
        result2 = compute_executable_forward_returns(prices, hold_days=10, cost_rate=0.004)
        mean1 = result["fwd_ret_10d_exec"].mean()
        mean2 = result2["fwd_ret_10d_exec"].mean()
        assert mean2 < mean1 + 0.01

    def test_end_of_sample_is_nan(self, prices):
        result = compute_executable_forward_returns(prices, hold_days=10)
    ...
    def test_mfe_mae_computed(self, prices):
        result = compute_executable_forward_returns(prices, hold_days=10)
    ...
    def test_no_future_leak(self, prices):
    ...
    def test_grouped_wrapper(self, prices):
        result = compute_forward_returns_grouped(prices, hold_days=10)
        assert "fwd_ret_10d_exec" in result.columns
        assert len(result) > 0
...
class TestBHSignificanceShrinkage:
    def test_insignificant_factor_gets_weak_weight(self, prices):
