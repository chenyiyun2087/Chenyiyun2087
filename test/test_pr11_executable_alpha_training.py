...
class TestExecutableIC:
    def test_ic_with_executable_labels(self, data):
        ...
        cal = sorted(prices["trade_date"].drop_duplicates().tolist())
        ic = compute_executable_ic(prices, sig, "rs_raw", cal, hold_days=10)
        assert len(ic) >= 0
    ...
    def test_ic_empty_on_missing_label_col(self, data):
        ...
        sig["x_raw"] = 0.0
        cal = sorted(prices["trade_date"].drop_duplicates().tolist())
        ic = compute_executable_ic(prices, sig, "x_raw", cal, hold_days=5)
    ...
    def test_executable_ic_differs_from_simple(self, data):
        ...
        cal = sorted(prices["trade_date"].drop_duplicates().tolist())
        ic_exec = compute_executable_ic(prices, sig, "rsi_raw", cal, hold_days=10, cost_rate=0.0015)
        ic_nocost = compute_executable_ic(prices, sig, "rsi_raw", cal, hold_days=10, cost_rate=0.0)
...
class TestFactorEffectiveness:
    ...
    def test_report_output_all_factors(self, data):
        ...
        cal = sorted(prices["trade_date"].drop_duplicates().tolist())
        report = generate_factor_report(prices, signals, cal)
        assert len(report) >= 2
...
class TestAlphaModelWithLabels:
    def test_rank_accepts_executable_labels(self, data):
        prices, scores = data
        cal = sorted(prices["trade_date"].drop_duplicates().tolist())
        labels = compute_executable_forward_returns(prices, cal, hold_days=10)
