...
TASKS = {
    ...
    "alpha_challenger_shadow_record": {
        "name": "Alpha Challenger Shadow 记录",
        "description": "v5.3 真前盲 Shadow：记录 T 日 challenger 候选信号与参考价 (15:30)",
        "script": "scripts/ops/run_daily_shadow.py --mode record",
        "last_run": "Never",
        "status": "Idle",
        "switched_day": False,
        "schedule_enabled": True,
    ...
    "alpha_challenger_shadow_reconcile": {
        "name": "Alpha Challenger Shadow 回填",
        "description": "v5.3 真前盲 Shadow：T+1 早盘回填实际开盘价与成交状态 (09:35)",
        "script": "scripts/ops/run_daily_shadow.py --mode reconcile",
        "last_run": "Never",
        "status": "Idle",
        "switched_day": False,
        "schedule_enabled": True,
...
def _verify_candle_diag_scan_result(started_at, finished_at, run_options=None):
    ...
    try:
            ...
            with conn.cursor() as cursor:
                cursor.execute(
                    """SELECT
                           ...
                           (SELECT COUNT(DISTINCT ts_code)
                              FROM tushare_stock.dwd_stock_daily_standard
                             WHERE trade_date = %s AND ts_code LIKE '%%.BJ') AS expected_bj_rows
        ...
        # symbols without a usable lookback window.  Use the target-day bar
        # universe as the historical baseline: current dim_stock contains
