...
> 评估时间：2026-07-27T00:19:40+08:00；策略：`production_governed_vol_position_v1_2b_dynamic_score`；release：`champion-v1-2b-dynamic-score-20260618`。
...
| 类别 | 门禁 | 状态 | 当前证据 | 修复动作 |
|---|---|---|---|---|
| 数据与发布身份 | `release_identity` | BLOCKED | 存在PENDING或身份不匹配 | 生成并冻结缺失快照，绑定新的不可变release证据包。 |
| 长周期回测 | `full_history` | BLOCKED | 2023-11-30至2026-06-17，615个交易日 | 使用正式PIT快照从2013年重跑全部市场状态和基础/压力场景。 |
| Walk-forward | `rolling_oos` | BLOCKED | status=FAILED; passed_windows=0 | 在冻结样本上完成带purge/embargo的滚动OOS，测试窗不得继续调参。 |
| 严格账本 | `strict_ledger` | BLOCKED | FAILED_REVALIDATION | 补齐公司行动和生命周期快照，生成逐release对账指标并通过严格账本Gate。 |
| 统计稳健性 | `statistical_robustness` | BLOCKED | 未发现与该release绑定的正式统计稳健性证据包 | 对完整OOS收益运行DSR、CPCV-PBO、Block Bootstrap和七因子归因。 |
| 成本、滑点与容量 | `execution_stress` | BLOCKED | 当前冻结回测滑点=0.00%，无完整容量矩阵 | 按50万至1000万元及全部成本/滑点组合重跑并保存拒单、冲击和回撤扩张。 |
