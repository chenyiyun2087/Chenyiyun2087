...
> 评估时间：2026-07-30T18:47:07+08:00；策略：`production_governed_vol_position_v1_2b_dynamic_score`；release：`champion-v1-2b-dynamic-score-20260618`。

## Executive Summary

- **结论：`NO_GO`。** 当前允许新增风险资金为 **0 元**，不得启用Canary。
- **冻结回测累计收益 57.23%、年化 20.41%、最大回撤 -25.52%。** 但样本仅覆盖 2023-11-30 至 2026-06-17，不满足alpha_v3自2018年以来的正式核心验证门禁。
...
| 长周期回测 | `full_history` | BLOCKED | 2023-11-30至2026-06-17，615个交易日；PR-C=BLOCKED | 使用正式PIT快照从2018年重跑全部市场状态和基础/压力场景；更早数据仅作可选扩展证据。 |
...
| 统计稳健性 | `statistical_robustness` | BLOCKED | alpha_v3 attribution=BLOCKED; factor_ic=BLOCKED | 对完整OOS收益运行DSR、CPCV-PBO、Block Bootstrap和七因子归因。 |
| 成本、滑点与容量 | `execution_stress` | BLOCKED | alpha_v3 execution_stress=BLOCKED | 按50万至1000万元及全部成本/滑点组合重跑并保存拒单、冲击和回撤扩张。 |
