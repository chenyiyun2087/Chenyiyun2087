...
def get_stock_list(context):
    ...
    q = query(valuation.code, valuation.circulating_market_cap) \
        .filter(valuation.code.in_(lev_list)) \
...
def filter_limitup_stock(context, stock_list):
    ...
    return [
        stock for stock in stock_list
        if stock in context.portfolio.positions.keys() or last_prices[stock][-1] < current_data[stock].high_limit
    ]
...
def filter_limitdown_stock(context, stock_list):
    ...
    return [
        stock for stock in stock_list
        if stock in context.portfolio.positions.keys() or last_prices[stock][-1] > current_data[stock].low_limit
    ]
...
def filter_new_stock(context, stock_list, d):
    ...
    return [
        stock for stock in stock_list
        if not yesterday - get_security_info(stock).start_date < datetime.timedelta(days=d)
    ]
