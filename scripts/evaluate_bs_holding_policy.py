...
def build_report(dataset_dir: Path, horizon: int = 20, model_dir: Path | None = None) -> dict:
    ...
    if model_bundle:
        panel = apply_bs_model_scores(panel, model_bundle)
    return {
...
def main() -> None:
    ...
    parser.add_argument("--model-dir", type=Path, default=None, help="Optional trained model directory used to score panel rows.")
    ...
    report = build_report(dataset_dir, horizon=args.horizon, model_dir=args.model_dir)
    if args.write:
