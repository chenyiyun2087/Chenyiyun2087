...
def main() -> None:
    ...
    p.add_argument("--data-cache-dir", required=True); p.add_argument("--corporate-action-snapshot", required=True); p.add_argument("--corporate-action-manifest", required=True); p.add_argument("--security-lifecycle-snapshot", required=True); p.add_argument("--security-lifecycle-manifest", required=True); p.add_argument("--start-date", default="2023-11-30"); p.add_argument("--end-date", default="2026-06-18")
    a = p.parse_args(); checkpoint = Path(a.checkpoint_dir) / a.run_id; checkpoint.mkdir(parents=True, exist_ok=True)
    payload = {"strategies": STRATEGIES, "start_date": a.start_date, "end_date": a.end_date, "execution_mode": "strict_t1_open_precommit", "checkpoint_every": a.checkpoint_every, "data_cache_dir": a.data_cache_dir, "corporate_action_snapshot": a.corporate_action_snapshot, "corporate_action_manifest": a.corporate_action_manifest, "security_lifecycle_snapshot": a.security_lifecycle_snapshot, "security_lifecycle_manifest": a.security_lifecycle_manifest}
    ...
    cmd.extend(["--corporate-action-snapshot", a.corporate_action_snapshot, "--corporate-action-manifest", a.corporate_action_manifest, "--security-lifecycle-snapshot", a.security_lifecycle_snapshot, "--security-lifecycle-manifest", a.security_lifecycle_manifest])
