...
def cscv_pbo(
    ...
    *,
    ...
    The best in-sample candidate is ranked against every candidate out of
    sample.  PBO is the fraction of splits whose OOS rank logit is non-positive.
    """
    ...
    try:
        ...
        for held_out in itertools.combinations(range(groups), held):
            ...
            # Bailey et al. CSCV: convert the selected strategy's OOS rank to
            # omega in (0, 1), then count negative rank logits as overfit.
            rank = 1 + int(np.sum(oos_sr < oos_sr[selected]))
            omega = float((rank - 0.5) / len(oos_sr))
            rank_logit = float(np.log(omega / (1.0 - omega)))
            loss = bool(rank_logit <= 0.0)
            losses += int(loss)
            rows.append({
...
def permutation_test(
    ...
    *,
    ...
    For a one-sample return series the null is generated by one random sign per
    contiguous 20-day block, preserving within-block dependence. ``null_values`` enables a
    ...
    try:
        ...
        if int(block_size) <= 0:
            raise StatisticalValidationError("permutation_block_size_invalid")
        mode = "block_sign_flip"
        ...
        if second is None:
            for _ in range(requested):
                signs = rng.choice(
                    np.array([-1.0, 1.0]),
                    size=int(np.ceil(len(observed_values) / int(block_size))),
                )
                sample = observed_values * np.repeat(signs, int(block_size))[:len(observed_values)]
        ...
        return _pass(
            ...
            mode=mode,
            block_size=int(block_size) if second is None else None,
