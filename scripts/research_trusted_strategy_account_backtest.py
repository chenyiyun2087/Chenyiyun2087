...
def _manifest_provenance(manifest: dict, *, snapshot_key: str, source_key: str) -> dict[str, object]:
    required = {"snapshot_schema_version", "dataset_version", "generated_at", snapshot_key, source_key}
...
def run_account_backtest(args: argparse.Namespace) -> dict:
    ...
    for spec in specs:
        ...
        use_strict_ledger = bool(
            ...
            and (
                ...
                or (getattr(args, "force_strict_ledger", False) and spec.name in CHALLENGER_STRATEGIES)
            )
        )
    ...
    out_dir = Path(requested_output_dir) if requested_output_dir else OUT_ROOT / datetime.now().strftime("%Y%m%d_%H%M%S_%f_trusted_account_backtest")
...
def main() -> None:
    ...
    parser.add_argument(
        "--force-strict-ledger",
        action="store_true",
        help="v5.2 research: run challenger strategies through the strict execution ledger (precommit T+1 open, order-level conservation, reconciliation in nav) instead of the account-level open-price path. Without it, challenger formal runs silently execute non-strict and derive PARTIAL_UNVERIFIED evidence.",
    )
