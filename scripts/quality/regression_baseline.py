...
def validate_baseline_payload(baseline: Mapping[str, Any]) -> None:
    ...
    metadata = _require_mapping(_get_required(baseline, "metadata", "baseline"), "baseline.metadata")
    for key in ("approved_by", "approved_at", "previous_baseline_sha256", "change_reason"):
        value = _get_required(metadata, key, "baseline.metadata")
        if not isinstance(value, str) or not value.strip():
            raise BaselineFormatError(f"baseline.metadata.{key} must be a non-empty string")
