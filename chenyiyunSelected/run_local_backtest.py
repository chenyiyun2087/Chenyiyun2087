...
from chenyiyunSelected.local_strategy_adapter import DBConfig, LocalHighDividendStrategy, StrategyConfig, TushareWarehouseProvider
    ...
    while cur <= end_dt:
        if cur.weekday() == 0:  # Monday
            try:
                ...
                plan[cur.strftime("%Y-%m-%d")] = picked["ts_code"].head(local_strategy.config.stock_num).tolist()
            except Exception:
                plan[cur.strftime("%Y-%m-%d")] = []
...
def main() -> None:
    ...
    picker = LocalHighDividendStrategy(TushareWarehouseProvider(db_cfg), StrategyConfig())
    ...
    config = BacktestConfig(initial_cash=args.initial_cash, commission_rate=0.0003, slippage_bps=20)
