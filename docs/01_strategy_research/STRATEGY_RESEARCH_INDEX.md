...
| 主题 | 目录/文件 | 当前状态 | 最新结论 |
|---|---|---|---|
| 可信全量池流动性策略 | `docs/tasks/20260512-full-pool-liquidity-strategy.md` | 持续迭代 | 当前飞书主推送切为 `baseline_full_liquidity_detail_vol_position`，`adaptive_market_style` 保留为市场/行业状态风控影子对照。 |
...
| 策略 | 类型 | 用途 | 备注 |
|---|---|---|---|
| `baseline_full_liquidity_detail_vol_position` | 生产主推送 | 收益优先近期冠军策略 | 当前飞书主推送策略，默认仓位 70%；最近 3 个月收益约 +36.71%、最大回撤约 -12.87%，但完整三年裸跑最大回撤约 -66.41%，必须保留风控对照。 |
| `adaptive_market_style` | 风控影子 | 市场风格自适应生产策略 | v2.2 继续用于每日市场/行业/量能状态检查和降仓参考：三年 T+1 账户级回测收益约 +42.09%，年化约 +11.36%，最大回撤约 -37.33%；AShare 增强默认最多补位 2 只。 |
