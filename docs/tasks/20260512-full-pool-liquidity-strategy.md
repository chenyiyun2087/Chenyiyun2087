...
| 2026-06-05 | vol_position 主推送切换 | 完成 | 按最新生产决策，飞书主推送策略从 `adaptive_market_style` 切为 `baseline_full_liquidity_detail_vol_position`，默认 `adaptive` 风险档直接映射该策略、目标仓位 70%。`adaptive_market_style` v2.2 不下线，保留为每日市场/行业/量能状态和降仓参考影子对照。`vol_position` 完整三年账户级回测收益约 +18.40%、年化约 +5.32%、最大回撤约 -66.41%；最近 3 个月收益约 +36.71%、最大回撤约 -12.87%，最近半年收益约 +84.99%、最大回撤约 -28.75%。 |
| 2026-06-05 | Sina 检测图片周清理任务 | 完成 | 新增 `scripts/ops/cleanup_sina_bs_detection_images.py`，按周五参考日删除上一周 `sina/bs_detection/SinaAppBS/config_1/YYYYMMDD/` 图片日期目录；脚本默认 dry-run，调度/Web 任务带 `--execute`。Web 任务中心新增“Sina检测图片周清理（周五）”，默认周五 22:05 执行且不依赖交易日；独立 `scheduler.py` 也接入同名任务。2026-06-05 dry-run 计划清理 `20260528`、`20260529` 两个目录，共 870 个文件。 |
